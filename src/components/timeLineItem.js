...
const Item = ({ data }) => (
  <Container
    ...
    style={{
      alignSelf: data.category.align,
      backgroundColor: GetBackground(data.category.tag),
    }}
  >
    <Row>
      <Text>{data.date}</Text>
      <Tag>{GetLogo(data.category.tag)}</Tag>
    </Row>
    <Title>{data.title}</Title>
    <Text>{data.text}</Text>
    {data.link && (
      <StyleLink href={data.link.url} target="_blank" rel="noopener noreferrer">
        {data.link.text}
      </StyleLink>
    )}
